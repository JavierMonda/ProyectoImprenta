...
<body>
 	...
 	<?php
		...
		if (isset($_POST["alta2"])){
			...
			$sql = "INSERT INTO tDepartamento VALUES ('$nombreDep','$emailDep');";
		...
		if (isset($_POST["guardar"]) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($nombreDep);$i++) {
			   	$emailDep[$i] = test_input($emailDep[$i]);
    			  
    			$j = 0;
    			$sql = ""; 
    			while ($j < count($seleccionar)) { 
    				if ($seleccionar[$j ++] == $nombreDep[$i]){

        				$sql = "UPDATE tDepartamento SET emailDep= '$emailDep[$i]'  
        				WHERE nombreDep='$nombreDep[$i]'";
    					if ($mysqli->query($sql)){
                			echo "Registro " .$nombreDep[$i] ." modificado satisfactoriamente";
    	...
    	if ((isset($_POST["eliminar"])) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($nombreDep);$i++) {
			   	$nombreDep[$i] = test_input($nombreDep[$i]);
    			$emailDep[$i] = test_input($emailDep[$i]);
        		...
        		while ($j < count($seleccionar)){
            		if ($seleccionar[$j ++] == $nombreDep[$i]){
                		$sql = "DELETE FROM tDepartamento WHERE nombreDep='$nombreDep[$i]'";
                	}
        		}  
	...
	?>
			<div class="form-group">
				<fieldset>
					<legend><span>Alta de Departamentos</span></legend>
					<form class="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
						
						<label for="nombreDep">Departamentos </label><a name="nombreDep"></a>
		    			<input class="form-control" tabindex="1" type="text" name="nombreDep" placeholder="Nombre Departamento" required>

		    			<label for="emailDep">Email </label><a name="emailDep"></a> 
		    			<input class="form-control" tabindex="2" type="text" name="emailDep" placeholder="Email Departamento" required>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
					...
					<div class="form-group">
						...
						?>
							<legend><span>Alta, Baja y Modificación de Departamentos</span></legend> 

							<form class="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">

								<table class="table-hover table-responsive table-striped">
									<tr><td colspan="6">Dar de alta un nuevo Departamento: </td></tr>
							...
							<?php
								while ($fila = $resultado -> fetch_assoc()){
									echo '
									<tr>
										<td><input type="checkbox" name="seleccionar[]" class="form-control" value="' .$fila['nombreDep'] .'"/></td>
										<td><input type="text" name="nombreDep[]" class="form-control" value="' .$fila['nombreDep'] .'" readonly></td>
										<td><input type="email" name="emailDep[]" class="form-control" value="' .$fila['emailDep'] .'"></td>
									</tr>';
