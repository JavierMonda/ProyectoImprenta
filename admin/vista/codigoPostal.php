...
<head>
    ...
    <title>Administración - Códigos Postales</title>
...
<body>
 	...
 	<?php
		...
		if (isset($_POST["alta2"])){
			...
			$sql = "INSERT INTO tCodigoPostal VALUES ('$cp','$localidad');";
		...
		if (isset($_POST["guardar"]) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($cp);$i++) {
			   	$localidad[$i] = test_input($localidad[$i]);
    			  
    			$j = 0;
    			$sql = ""; 
    			while ($j < count($seleccionar)) { 
    				if ($seleccionar[$j ++] == $cp[$i]){

        				$sql = "UPDATE tCodigoPostal SET localidad= '$localidad[$i]'  
        				WHERE cp='$cp[$i]'";
    					if ($mysqli->query($sql)){
                			echo "Registro " .$cp[$i] ." modificado satisfactoriamente";
    	...
    	if ((isset($_POST["eliminar"])) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($dniCif);$i++) {
        		...
        		while ($j < count($seleccionar)){
            		if ($seleccionar[$j ++] == $cp[$i]){
                		$sql = "DELETE FROM tCodigoPostal WHERE cp='$cp[$i]'";
                	}
        		}  
	...
	?>
			<div class="form-group">
				<fieldset>
					<legend><span>Alta de Códigos Postales</span></legend>
					<form class="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
						
						<label for="cp">Código Postal </label><a name="cp"></a>
		    			<input class="form-control" tabindex="1" type="text" name="cp" placeholder="Ej: 35600" required>

		    			<label for="localidad">Localidad </label><a name="localidad"></a> 
		    			<input class="form-control" tabindex="2" type="text" name="localidad" placeholder="Localidad" required>
                ...
                <div class="col-lg-12">
					...
					<div class="form-group">
						...
						<?php 
							...
							$sql = "SELECT * FROM  tCodigoPostal";						
							$resultado = $mysqli -> query($sql);						
						?>
							<legend><span>Alta, Baja y Modificación de Códigos Postales</span></legend> 

							<form class="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">

								<table class="table-hover table-responsive table-striped">
									<tr><td colspan="6">Dar de alta un nuevo Código Postal: </td></tr>
							...
							<?php
								while ($fila = $resultado -> fetch_assoc()){
									echo '
									<tr>
										<td><input type="checkbox" name="seleccionar[]" class="form-control" value="' .$fila['cp'] .'"/></td>
										<td><input type="text" name="cp[]" class="form-control" value="' .$fila['cp'] .'" readonly></td>
										<td><input type="text" name="localidad[]" class="form-control" value="' .$fila['localidad'] .'"></td>
									</tr>';
