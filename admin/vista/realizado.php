...
<head>
    ...
    <title>Administración - Realización de productos</title>
...
<body>
 	...
 	<?php
		...
		if (isset($_POST["alta2"])){
			...
			$idProductoRealiza = $_POST["idProductoRealiza"];
			$dniEmpRealiza = $_POST["dniEmpRealiza"];

			//Se describe la inserción de datos en SQL
			$sql = "INSERT INTO tRealiza VALUES ('$idProductoRealiza','$dniEmpRealiza');";
		...
		if (isset($_POST["guardar"]) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($idProductoRealiza);$i++) {
			   	$dniEmpRealiza[$i] = test_input($dniEmpRealiza[$i]);
    			  
    			$j = 0;
    			$sql = ""; 
    			while ($j < count($seleccionar)) { 
    				if ($seleccionar[$j ++] == $idProductoRealiza[$i]){

        				$sql = "UPDATE tRealiza SET dniEmpRealiza= '$dniEmpRealiza[$i]'  
        				WHERE idProductoRealiza='$idProductoRealiza[$i]'";
    					if ($mysqli->query($sql)){
                			echo "Registro " .$idProductoRealiza[$i] ." modificado satisfactoriamente";
    	...
    	if ((isset($_POST["eliminar"])) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($dniCif);$i++) {
			   	$idProductoRealiza[$i] = test_input($idProductoRealiza[$i]);
    			$dniEmpRealiza[$i] = test_input($dniEmpRealiza[$i]);
        		...
        		while ($j < count($seleccionar)){
            		if ($seleccionar[$j ++] == $idProductoRealiza[$i]){
                		$sql = "DELETE FROM tRealiza WHERE idProductoRealiza='$idProductoRealiza[$i]'";
                	}
        		}  
            ...
            <div class="navbar-default sidebar" role="navigation">
                <div class="sidebar-nav navbar-collapse">
                    <ul class="nav" id="side-menu">
                        ...
                        <li>
                            <a href="realizado.php"><i class="fa fa-edit fa-fw"></i> Realización de Productos</a>
                        </li>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Dashboard
                    	<small>Realización de productos</small>
                    </h1>
                ...
                <div class="col-lg-12">
                    ...
                    ?>
                            <div class="form-group">
                                <fieldset>
                                    <legend><span>Alta</span></legend>
                                    <form class="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                                        
                                        <label for="idProductoRealiza">Productos </label><a name="idProductoRealiza"></a>
                                        <input class="form-control" tabindex="1" type="number" name="idProductoRealiza" placeholder="Productos" required>

                                        <label for="dniEmpRealiza">Empleado que lo realiza </label><a name="dniEmpRealiza"></a> 
                                        <input class="form-control" tabindex="2" type="text" name="dniEmpRealiza" placeholder="Empleado que lo realiza" required>
                ...
                <div class="col-lg-12">
					...
					<div class="form-group">
						...
						<?php 
							...
							$sql = "SELECT * FROM  tRealiza";						
							$resultado = $mysqli -> query($sql);						
						?>
							<legend><span>Alta, Baja y Modificación de la realización</span></legend> 

							<form class="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">

								<table class="table-hover table-responsive table-striped">
									<tr><td colspan="6">Dar de alta un nuevo registro: </td></tr>
									<tr><td colspan="6"><button type="submit" name="alta" class="btn btn-default" />Alta</button></td></tr>
									<tr>
										<th>Seleccionar</th>
										<th>Id del Producto</th>
										<th>Empleado que lo realiza</th>
									</tr>
							<?php
								while ($fila = $resultado -> fetch_assoc()){
									echo '
									<tr>
										<td><input type="checkbox" name="seleccionar[]" class="form-control" value="' .$fila['idProductoRealiza'] .'"/></td>
										<td><input type="number" name="idProductoRealiza[]" class="form-control" value="' .$fila['idProductoRealiza'] .'" readonly></td>
										<td><input type="text" name="dniEmpRealiza[]" class="form-control" value="' .$fila['dniEmpRealiza'] .'"></td>
									</tr>';
