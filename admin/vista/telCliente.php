...
<head>
    ...
    <title>Administración - Teléfonos de Clientes</title>
...
<body>
 	...
 	<?php
		...
		if (isset($_POST["alta2"])){
			...
			$dniCif_Tf = $_POST["dniCif_Tf"];

			//Se describe la inserción de datos en SQL
			$sql = "INSERT INTO tCliente_Tf VALUES ('$telefonoCl','$dniCif_Tf');";
		...
		if (isset($_POST["guardar"]) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($telefonoCl);$i++) {
			   	$dniCif_Tf[$i] = test_input($dniCif_Tf[$i]);
    			  
    			$j = 0;
    			$sql = ""; 
    			while ($j < count($seleccionar)) { 
    				if ($seleccionar[$j ++] == $dniCif_Tf[$i]){

        				$sql = "UPDATE tCliente_Tf SET telefonoCl= '$telefonoCl[$i]'  
        				WHERE dniCif_Tf='$dniCif_Tf[$i]'";
    					if ($mysqli->query($sql)){
                			echo "Registro " .$telefonoCl[$i] ." modificado satisfactoriamente";
    	...
    	if ((isset($_POST["eliminar"])) && (isset($_POST["seleccionar"]))) {
			...
			for ($i=0;$i < count($dniCif);$i++) {
			   	$telefonoCl[$i] = test_input($telefonoCl[$i]);
    			$dniCif_Tf[$i] = test_input($dniCif_Tf[$i]);
        		...
        		while ($j < count($seleccionar)){
            		if ($seleccionar[$j ++] == $dniCif_Tf[$i]){
                		$sql = "DELETE FROM tCliente_Tf WHERE dniCif_Tf='$dniCif_Tf[$i]' 
                        AND telefonoCl='$telefonoCl[$i]'";
			...
			<div class="form-group">
				<fieldset>
					...
					<form class="form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
						...
						<label for="telefonoCl">Teléfono </label><a name="telefonoCl"></a>
		    			<input class="form-control" tabindex="1" type="text" name="telefonoCl" placeholder="Ej: 928000111" required>

		    			<label for="dniCif_Tf">Cliente </label><a name="dniCif_Tf"></a> 
		    			<input class="form-control" tabindex="2" type="text" name="dniCif_Tf" placeholder="Cliente" required>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Dashboard
                    	<small>Teléfonos de Clientes</small>
                    </h1>
						...
						?>
							...
							<?php
								while ($fila = $resultado -> fetch_assoc()){
									echo '
									<tr>
										<td><input type="checkbox" name="seleccionar[]" class="form-control" value="' .$fila['dniCif_Tf'] .'"/></td>
                                        <td><input type="text" name="dniCif_Tf[]" class="form-control" value="' .$fila['dniCif_Tf'] .'" readonly></td>
										<td><input type="text" name="telefonoCl[]" class="form-control" value="' .$fila['telefonoCl'] .'"></td>
